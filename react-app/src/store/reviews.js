...
export const editReviewSelector = (state) => state.reviews.edit;
...
export function setEditReview(review) {
  return {
    type: EDIT_REVIEW,
    review,
  };
...
// fetch a user review for editing by its id thunk
export const fetchReview = (reviewId) => async (dispatch) => {
  const res = await fetch(`/api/reviews/${reviewId}`);
  if (res.ok) {
    const data = await res.json();
    dispatch(setEditReview(data));
  }
};
